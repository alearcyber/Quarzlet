...
def addquiz():
    ...
    try:
        ...
        for q in question_list:
            valid = ("question" in q.keys()) and ("correctanswer" in q.keys()) and ("badanswers" in q.keys())
            if not valid:
    ...
    try:
        ...
        jsonify({"status": 0, "Message": "There was an issue adding the quiz to the database. It was NOT added"})
...
def add_new_quiz(data):
    ...
    for question in questions:
        ...
        for wrong_answer in wrong_answers:
            query(f"""INSERT INTO wrong (question_text, answer_text) VALUES (?, ?)""", (q, wrong_answer))
